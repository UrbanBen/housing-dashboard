...
const logger = createComponentLogger('BAMonthlyCard');

interface BAMonthlyCardProps {
...
export function BAMonthlyCard({ selectedLGA }: BAMonthlyCardProps) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        logger.error('[BAMonthlyCard] Error', { error: err });
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      return (
        <div className="bg-card border-2 border-purple-500 rounded-lg p-3 shadow-lg">
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <CalendarRange className="h-6 w-6 text-[#6366f1]" />
            <div>
              <CardTitle className="text-xl text-[#6366f1]">Monthly BA Summary</CardTitle>
              <p className="text-sm text-muted-foreground mt-1">
                {selectedLGA ? `Last 12 months - ${selectedLGA.name}` : 'Select an LGA'}
              </p>
            </div>
          </div>
        </div>
      ...
      <CardContent>
        ...
        {!loading && !error && data.length === 0 && (
          <div className="flex items-center justify-center h-64 text-muted-foreground">
            {selectedLGA ? 'No data available for this LGA' : 'Select an LGA to view monthly summary'}
          </div>
        )}
            ...
            <div className="grid grid-cols-3 gap-3 mb-4">
              <div className="bg-blue-500/5 border border-blue-500/10 rounded-lg p-3 text-center hover:bg-blue-500/10 transition-all">
                <div className="text-2xl font-bold text-blue-600 dark:text-blue-400">
                  {summary?.total_last_12_months?.toLocaleString() || 0}
                </div>
                <div className="text-xs text-muted-foreground">12-Month Total</div>
              </div>
              ...
              <div className="bg-green-500/5 border border-green-500/10 rounded-lg p-3 text-center hover:bg-green-500/10 transition-all">
                <div className="text-2xl font-bold text-green-600 dark:text-green-400">
                  {summary?.avg_per_month ? Number(summary.avg_per_month).toFixed(1) : '0'}
                </div>
              ...
              <div className="bg-purple-500/5 border border-purple-500/10 rounded-lg p-3 text-center hover:bg-purple-500/10 transition-all">
                ...
                <div className="text-xs text-muted-foreground">Months Recorded</div>
              </div>
            </div>
