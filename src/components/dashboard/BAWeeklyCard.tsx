...
export function BAWeeklyCard({ selectedLGA }: BAWeeklyCardProps) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        logger.error('[BAWeeklyCard] Error', err );
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <CardTitle className="text-xl text-[#6366f1]">Weekly BA Trends</CardTitle>
              <p className="text-sm text-muted-foreground mt-1">
                {selectedLGA ? `Last 12 weeks - ${selectedLGA.name}` : 'Select an LGA'}
              </p>
            </div>
          </div>
        </div>
      ...
      <CardContent>
        ...
        {!loading && !error && data.length === 0 && (
          <div className="flex items-center justify-center h-64 text-muted-foreground">
            {selectedLGA ? 'No data available for this LGA' : 'Select an LGA to view weekly trends'}
          </div>
        )}
            ...
            <div className="grid grid-cols-3 gap-3 mb-4">
              ...
              <div className="bg-green-500/5 border border-green-500/10 rounded-lg p-3 text-center hover:bg-green-500/10 transition-all">
                <div className="text-2xl font-bold text-green-600 dark:text-green-400">
                  {summary?.avg_per_week ? Number(summary.avg_per_week).toFixed(1) : '0'}
                </div>
              ...
              <div className="bg-purple-500/5 border border-purple-500/10 rounded-lg p-3 text-center hover:bg-purple-500/10 transition-all">
                ...
                <div className="text-xs text-muted-foreground">Weeks Recorded</div>
              </div>
            </div>
