...
import { X, Database, Settings, Save, FolderOpen, Building2, Link, Home, Target, CheckCircle } from "lucide-react";
...
export interface LGAMetricsConfig {
  host: string;
  port: number;
  database: string;
  user: string;
  ...
  dataItems: {
    buildingApprovals: DataItemConfig;
    housingTarget: DataItemConfig;
    daApprovals: DataItemConfig;
  };
}

interface LGAMetricsConfigFormProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (config: LGAMetricsConfig) => void;
  currentConfig: LGAMetricsConfig | null;
}
...
export function LGAMetricsConfigForm({
  isOpen,
  onClose,
  onSave,
  currentConfig
}: LGAMetricsConfigFormProps) {
  const [config, setConfig] = useState<LGAMetricsConfig>({
  ...
  const dataItemIcons = {
    buildingApprovals: Home,
    housingTarget: Target,
    daApprovals: CheckCircle
  };
          ...
          <CardHeader className="pb-4 border-b border-border sticky top-0 bg-background z-10">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <CardTitle className="text-xl">NSW Housing Metrics Configuration</CardTitle>
