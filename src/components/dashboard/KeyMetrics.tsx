...
export function KeyMetrics({ selectedLGA: externalSelectedLGA, isAdminMode = false, onAdminClick }: KeyMetricsProps) {
  ...
  const calculateApprovalRate = () => {
    ...
    const pop = Number(censusData.population_2026_proj);
    if (isNaN(pop) || pop <= 0) return null;
    return ((Number(data.buildingApprovals) / pop) * 1000).toFixed(2);
  };
  ...
  const calculateValuePerCapita = () => {
    ...
    const pop = Number(censusData.population_2026_proj);
    if (isNaN(pop) || pop <= 0) return null;
    return Math.round(Number(data.valueTotalRes) / pop);
  };
