...
test('homepage loads correctly', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Urban Analytics' })).toBeVisible();

  // Check that the description is present
  await expect(page.getByText('Advanced urban planning analytics platform')).toBeVisible();
