...
const backupFile = path.join(__dirname, 'public', 'australia-states-pre-75.backup.geojson');

const SCALE_FACTOR = 0.75; // Reduce to 75%

console.log('📐 Scaling ACT to 75% of current size...\n');
...
if (nswFeature && nswFeature.geometry.type === 'MultiPolygon') {
  ...
  if (foundHole) {
    console.log('   ✓ NSW ACT hole scaled to 75%');
  } else {
...
console.log('\n💾 Saving scaled GeoJSON...');
fs.writeFileSync(outputFile, JSON.stringify(data, null, 2));

console.log('\n✅ ACT scaling to 75% complete!');
console.log(`\n📁 Files:`);
console.log(`   Backup: ${backupFile}`);
console.log(`   Modified: ${outputFile}`);
console.log(`\n💡 ACT is now 75% of its previous size.`);
